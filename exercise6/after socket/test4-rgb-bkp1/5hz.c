...
#define DIFF_THRESH (0.2) // In percentage for 10Hz
...
void *capture_image(void *threadp)
{
	...
	if (thread_param->threadIdx == 1)
	{
		...
		close(fd);
		printf("Exiting capture service\n");
		pthread_exit(NULL);
...
void *soc_image(void *threadp)
{
	...
	while(1)
	{
		...
		printf("Socket send frame start : %d\n", fts);
		...
		if (write_size == read_size) printf("frame %d transferred\n", fts);


		printf("Socket send frame complete : %d", fts);
		close(fd2);
		sched_yield();
	}
...
{
	mask_sig(); printf("Sequencer waits on sigwait now\n");
...
int main (int argc, char *argv[])
{
   ...
   struct sched_param main_param;
	...
	for(i = 0; i < NR_THREADS; i++)
	{
		...
		if (policy == SCHED_OTHER) printf("  NRT Service thread index %d policy is SCHED_OTHER\n", i);
