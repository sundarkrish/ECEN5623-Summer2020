...
typedef struct
{
	char c_buffer[(PIXELS*4)*6]; // Takes this space after YUV to RGB conversion
	char g_buffer[PIXELS]; // Takes this space after YUV to gray conversion
	char im_header[500];
	int set_no;
...
void *select_image(void *threadp)
{
   ...
   while (1)
   {
		...
		save_image(PGM_fname, good_payload->im_header, good_payload->c_buffer, ((PIXELS*4)*6));		
#endif
...
void *sequencer(void *threadp)
{
	...
	{	
		...
		timeout = (((seq_timeout.tv_sec * 1000000000) + seq_timeout.tv_nsec) + timeout_calc) - CALIB;
