...
//  not set.
//  Code source: http://ecee.colorado.edu/~ecen5623/ecen/ex/Linux/code/example-sync-updated-2/
//
//  warnings in pthread3.c and pthreadok.c corrected here.Following are the changes
//  Code modification: (main change) H,M and L all are SCHED_FIFO, start-service is SCHED_OTHER to let L task run and not fully  before H task
//                     gettimeofday() supports "struct timeval" and not "struct timespec" - corrected here
//                     struct timeval has tv_usec instead of tv_nsec - corrected here
//                     gettimeofday() lib call header <sys/time.h> added here
//                    
#define _GNU_SOURCE
