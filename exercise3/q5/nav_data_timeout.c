 ...
 *
 *        OS may have a ready queue for each core or a common ready queue for all cores, in SMP
 *        So better to use AMP in SMP using affinity
 */
...
void *thread_handler_1(void *arg)
{
    ...
    for (cnt = 0; cnt < COUNT; cnt++)
    {
        ...
        // The time to update new data is random.Here it takes anywhere between
        // 1 and 15 seconds
        sleep_cnt = 0;
        pthread_mutex_lock(mtx);
        delay_time.tv_sec = ((rand() % 15) + 1) ;
        do
        {
            nanosleep(&delay_time, &rem_time);
            delay_time.tv_sec = rem_time.tv_sec;
            delay_time.tv_nsec = rem_time.tv_nsec;
            sleep_cnt++;
...
// The thread waits on the mutex lock no more than 10seconds.
void *thread_handler_2(void *arg)
{
    int cnt = 0, complete_flag = 0, rv;
    ...
    while (1)
    {
        ...
        timeout.tv_sec += 10;
        rv = pthread_mutex_timedlock(mtx, &timeout); // Should supply absolute time
        if (rv == 0)
        {
        ...
        if (rv == ETIMEDOUT)
        {
            ...
            printf("No new data available.Will try again.\n");
        }  

        

        if (cnt == 100) pthread_exit(NULL); 
