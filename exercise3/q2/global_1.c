 ...
 *        Single function handler, thread specific task and variable access using if-block
 */
...
void *thread_handler(void *arg)
{
    ...
    if (thread_param->thread_idx == 0)
        printf("cpu: %d -- thread: 0 \n", sched_getcpu());

    if (thread_param->thread_idx == 1)
        printf("cpu: %d -- thread: 1 \n", sched_getcpu());
